...
```json
POST indesName/_delete_by_query
{
  "query": {
    "match_all": {}
  }
...
#### 修改索引结构
加新字段与上面的创建索引结构一样。
es index创建好字段后，不支持字段类型的修改，只能是创建新的index之后，把老的数据复制到新的里
...
可通过别名映射的方式，代码一直用别名，es里指定别名所映射的index，来实现无缝切换
...
#### 路由
es通过routing来计算数据的分片
[具体参考](https://www.cnblogs.com/caoweixiong/p/12029789.html)
...
```
    ...
    > 警惕使用from深度分页，es是分布式的，请求节点归集所有节点的结果并排序之后返回分页结果，注意是所有数据，举例来说：假设在一个有5个主分片的索引中搜索。当我们请求结果的第一页（结果 1到10）时，每个分片产生自己最顶端10个结果然后返回它们给请求节点(requesting node)，它再排序这所有的50个 结果以选出顶端的10个结果。
...
## kibana显示日志时间
kibana默认时间戳是使用的是数据进入es的时间，会和实际的日志时间有一定的时间差，通过设置logstash的pipeline配置，可以用日志时间替换时间戳

```
  #grok解析日志，提取日志里的时间到log.timestamp字段
  grok {
      match => {
          "message" => "%{TIMESTAMP_ISO8601:log.timestamp}%{SPACE}%{LOGLEVEL:log.level}%{SPACE}\[(?<log.thread>.*?)\]%{SPACE}(?<log.logger>.*?)\:(?<log.line>\d*)%{SPACE}\-%{SPACE}(?<log.message>(?m).*)"
      tag_on_failure => ["grok_parse_failure"]
  }
  #使用log.timestamp替换@timestamp，并指定时区，不指定时区的话kibana默认用的utc时间
  date {
  ...
  mutate {
    rename => { "log.thread" => "[log][thread]" }
    #此时log.timestamp已经赋值给了@timestamp，所以删除
    remove_field => ["log.timestamp"]
    #rename => { "log.timestamp" => "[log][timestamp]" }
    rename => { "log.level" => "[log][level]" }
    rename => { "log.logger" => "[log][logger]" }
    rename => { "log.message" => "[log][message]" }
    rename => { "log.line" => "[log][line]" }
  }
```
